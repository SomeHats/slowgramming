...
import { assertExists, delay, spawnUnknownSwitchCaseError } from './util';

interface Visualizer {
  setActiveLine(line: number): void;
  highlightAstNode(node: ESTree.Node): void;
  clearHighlight(): void;
  hideChars(start: number, count: number): void;
}
...
const evaluateAdditionOperator = async (
  ...
  ctx.visualizer.highlightAstNode(node);
  await delay(500);
  ...
  if (JS.type(lPrim) === 'String' || JS.type(rPrim) === 'String') {
    ...
    if (lStr instanceof JS.StringValue && rStr instanceof JS.StringValue) {
      console.log('eval +, string:', lStr, rStr);
      ctx.visualizer.clearHighlight();
      return new JS.StringValue(lStr.value + rStr.value);
    } else {
  ...
  if (lNum instanceof JS.NumberValue && rNum instanceof JS.NumberValue) {
    console.log('eval +, number:', lNum, rNum);
    ctx.visualizer.clearHighlight();
    return new JS.NumberValue(lNum.value + rNum.value);
  } else {
